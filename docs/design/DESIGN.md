...
## Foundations
- **Цвета и роли:** Нативные переменные темы DSH (`--dsw-alias-*`). Полупрозрачные статусные фоны бейджей и алертов строятся через динамический `color-mix(in srgb, var(--dsw-alias-state-*-primary) X%, transparent)`, что гарантирует автоматическую адаптацию под тёмную и светлую темы без хардкода rgba/hex (#238).

    ...
    defineTool({
      ...
      async execute(args, exec) {
        ...
        const providers = makeProviders(
          ...
          { prompt: effectivePrompt, size, format, seed: args.seed, signal: exec.signal, negativePrompt: args.negative_prompt, guidanceScale: args.guidance_scale, source, mask, strength: args.strength, quality: args.quality, aspectPixels },
        )
        ...
        if (batchPrompts) {
          for (let i = 0; i < batchPrompts.length; i += 1) {
            ...
            const cached = cfg.cacheBySeed ? await cachedResult(findCached(historyEntries, seedBase + i, text)) : undefined

...
export function assertBudgetAvailable(estimatedCost, dailyBudgetUsd) {
  ...
  const current = loadDailySpend()
  const projected = +(current.totalSpendUsd + cost).toFixed(4)
  if (projected > budget) {
    throw new Error(
      `Daily image generation budget exceeded: current spend $${current.totalSpendUsd.toFixed(2)} + estimated $${cost.toFixed(2)} exceeds limit $${budget.toFixed(2)} (Settings → Image generation → dailyBudgetUsd)`
    )
  }
  return true
}

/**
 * Возвращает список кандидатов для имени ключа API с учётом известных алиасов (напр. FAL_API_KEY <-> FAL_KEY).
 */
  ...
  if (typeof e === 'string') {
    ...
    if (typeof e.message === 'string' && e.message && e.message !== '[object Object]') {
      ...
      if (typeof e.error === 'string') msg = e.error
        ...
        try { msg = JSON.stringify(e.error) } catch {}
      }
...
/**
 * Разрешает размер для подписочных сервисов (codex / grok).
 * Понимает именованные размеры (landscape_16_9 и др.), пропорции (16:9, 3:2, 9:16 и др.)
 * и размеры в пикселях [width, height], исключая дефолтный квадрат при запросе соотношений сторон.
 */
...
/** Прямой вызов редактирования (inpainting / img2img) через выбранного провайдера. */
...
/** Прямой вызов вариаций изображения через выбранного провайдера. */

...
function FalImageCard(props) {
    ...
    function apply(ctx) {
      ...
      // Карточка во вкладке «Настройки → Плагины → Настройки плагинов» (слот settings.plugin.item).
      // Ключ слота обязан равняться пространству настроек (SETTINGS_NS):
      // вкладка перебирает пространства и отрисовывает карточку.
      // Раздел верхнего уровня settings.section исключён, чтобы настройки не
      // засоряли общий боковой список настроек.
      // Отложенная регистрация через ctx.slots.inject предотвращает ошибки монтирования,
      // так как дочерние слоты страниц DSH UI объявляются родительскими компонентами.

...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function apply(ctx) {
      ...
      // Свой раздел в боковом списке настроек, а не карточка во вкладке
      // «Настройки плагинов».
      //
      // Ту вкладку рисует ядро, и в ней карточка появляется только если
      // namespace успел попасть в общий вид настроек. У части установок эта
      // вкладка остаётся пустой — плагины при этом грузятся штатно, ошибок нет,
      // и повлиять на неё из плагина нечем. Собственный раздел рисуем мы сами,
      // рядом с Vision и Голосом, и он от этой вкладки не зависит.
      //
      // Заодно отпадает возня с keyed-слотом: settings.section принимает id на
      // всех версиях ядра, и держать сразу id и key больше не нужно.

...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const FIELDS = [
      ...
      { field: 'provider', spec: selectField('provider', PROVIDERS), kind: 'select', options: PROVIDERS, labelKey: 'f.provider', hintKey: 'f.providerHint', providerNeeds: { fal: 'ключ FAL', custom: 'адрес + ключ', codex: 'подписка ChatGPT', grok: 'подписка Grok', local: 'адрес сервера' } },
    ...
    function Field(props) {
      ...
      if (entry.kind === 'select') {
        ...
        const need = entry.providerNeeds && entry.providerNeeds[state.text] ? ` (нужно: ${entry.providerNeeds[state.text]})` : ''
...
function FalImageCard(props) {
      ...
      const repeatText = (() => {
        ...
        if (seed !== undefined) parts.push(`Сид: ${seed}`)
      ...
      const onReroll = () => {
        ...
        sendActionPrompt(`Сгенерируй заново с другим сидом (${nextSeed}): ${prompt || repeatText}`)
      }

      const onUpscale = () => {
        const ref = parsed.attachment ? parsed.attachment.attachmentId : (parsed.url || '')
        sendActionPrompt(`Увеличь разрешение этого изображения в 2 раза: upscale_image(image: "${ref}")`)
      }

      const onRemoveBg = () => {
        const ref = parsed.attachment ? parsed.attachment.attachmentId : (parsed.url || '')
        sendActionPrompt(`Удали фон у этого изображения: remove_background(image: "${ref}")`)
      }
      ...
      if (args.originalPrompt && args.originalPrompt !== prompt) {
        body.push(react.createElement('div', { className: 'fal-meta', key: 'orig' }, `Исходный запрос: ${args.originalPrompt}`))
      }
      if (failed) {
      ...
      if (!running && !failed) {
        ...
        btns.push(react.createElement('button', {
          ...
          onClick: onReroll,
          title: 'Перегенерировать со следующим сидом'
        }, '🔄 Re-roll'))
      ...
      if (!running && !failed) {
        if (editing) {
          body.push(react.createElement('input', {
            ...
            className: 'fal-input',
            value: editText,
            placeholder: prompt || 'Новый промпт',
